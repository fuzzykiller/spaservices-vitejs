...
namespace SpaServices.ViteJsDevServer
{
    public static class ViteJsDevServerMiddleware
    {
        private const string LogCategoryName = "ViteJsDevServer";
        ...
        public static void Attach(ISpaBuilder spaBuilder, string scriptName)
        {
            ...
            var portTask = StartViteJsServerAsync(sourcePath, scriptName, pkgManagerCommand, devServerPort, logger, diagnosticSource, applicationStoppingToken);
            ...
            spaBuilder.UseProxyToSpaDevelopmentServer(() =>
            {
                ...
                return targetUriTask.WithTimeout(timeout, "The Vite.js server did not start listening for requests " +
                                                          $"within the timeout period of {timeout.TotalSeconds} seconds. " +
                                                          "Check the log output for error information.");
            });
        }

        private static async Task<int> StartViteJsServerAsync(
            string sourcePath, string scriptName, string pkgManagerCommand, int portNumber, ILogger logger, DiagnosticSource diagnosticSource, CancellationToken applicationStoppingToken)
        {
            ...
            logger.LogInformation($"Starting Vite.js server on port {portNumber}...");

            var scriptRunner =
                new NodeScriptRunner(sourcePath, scriptName,
                    Invariant($"--open false --clearScreen false --host 127.0.0.1 --port {portNumber} --strictPort"), null, pkgManagerCommand, diagnosticSource, applicationStoppingToken);
            ...
            using (var stdErrReader = new EventedStreamStringReader(scriptRunner.StdOut))
            {
                ...
                catch (EndOfStreamException ex)
                {
                    throw new InvalidOperationException(
                        ...
                        $"Vite.js server was listening for requests. The error output was: " +
